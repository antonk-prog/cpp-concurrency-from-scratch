...
В Concurrency TS есть `std::experimental::atomic_shared_ptr<T>` — атомарная
обёртка, которая обрабатывает подсчёт ссылок правильно и без явных вызовов
`atomic_load`/`atomic_store`. Но, как и другие атомарные типы, может не быть
lock-free на конкретной платформе.
...
|------|-----|-----------|--------------------|
| [7.1. Lock-free стек через CAS](labs/lab-07-01-lock-free-stack/task.md) | напиши с нуля | средняя | реализуй `push`/`pop` стека на CAS с `shared_ptr<T>` (утечка узлов допустима) |
| [7.2. Почини публикацию узла до готовности](labs/lab-07-02-publish-before-ready/task.md) | найди и почини | средняя | исправь `push`, который публикует узел в `head` до заполнения данных — гонка |
| [7.3. Проблема ABA](labs/lab-07-03-aba-problem/task.md) | предскажи вывод | сложная | объясни сценарий ABA в стеке с переиспользованием узлов и защиту счётчиком |