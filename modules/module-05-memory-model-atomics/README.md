...
**`memory_order_release`** («освобождение») ставится на **запись**. Она
обещает: всё, что поток записал в память *до* неё, увидят те, кто прочитает
записанное значение.

**`memory_order_acquire`** («захват») ставится на **чтение** и делает обратное:
поток, прочитавший значение, гарантированно увидит все записи, сделанные
потоком-освободителем до его `release`.

В паре они работают как «мост» между потоками: `release` отдаёт данные,
`acquire` их принимает. В спин-блокировке это и превращает голый флаг во
взаимное исключение: `unlock()` через `release` публикует всё, что поток успел
изменить под защитой, а следующий `lock()` через `acquire` обязан эти изменения
увидеть.
