...
напечатает либо `1,2`, либо `2,1`, но гарантировать нельзя. Причина — по
стандарту порядок вычисления аргументов функции не определён, а компилятор
волен перемешать код и попытаться оптимизировать (например, вычислить вызовы
в любом порядке или переиспользовать результат). Между операциями в одной
инструкции нет отношения «происходит до», если они не связаны (например,
...
Важно: начиная с C++17 стандарт гарантирует, что UB здесь нет, — вычисление
аргументов функции неопределённо упорядочено (indeterminately sequenced):
порядок не зафиксирован, но побочные эффекты выполняются последовательно,
поэтому программа всегда напечатает либо `1,2`, либо `2,1`. До C++17 два
неупорядоченных изменения одной и той же переменной (`++i` в двух вызовах)
были неопределённым поведением, и при оптимизации компилятор мог «сломать»
такую программу.
...
используется блокировка. Вот пример вызова — для встроенных типов и для
...
int main() {
    ...
    std::atomic<Pair> point{};

    std::cout << std::boolalpha
              << "atomic<int>:  " << counter.is_lock_free() << "\n"
              << "atomic<bool>: " << ready.is_lock_free() << "\n"
              << "atomic<Pair>: " << point.is_lock_free() << "\n";
}
```

На обычном x86-64 программа напечатает:
...
`Pair` состоит из двух `int` и занимает 8 байт — ровно столько, сколько умеет
одна аппаратная атомарная операция, поэтому такая структура свободна
от блокировок.

Теперь добавим в структуру ещё одно поле:
...
И тут тот же код **не соберётся**, если линковать только с `-pthread`:

```
undefined reference to `__atomic_is_lock_free'
```

Почему? `is_lock_free()` — не константа, а проверка **во время выполнения**:
за неё отвечает вспомогательная функция `__atomic_is_lock_free` из библиотеки
`libatomic`. Пока размер типа совпадает с аппаратной шириной (как у `Pair` —
8 байт), компилятор знает ответ заранее и просто подставляет `true`, и никакая
библиотека не нужна. Но после добавления `bool` структура занимает 12 байт —
аппаратной атомарной операции такого размера не существует. Уверенности
у компилятора нет, поэтому он оставляет настоящий вызов
`__atomic_is_lock_free`, а этой функции в подключаемых по умолчанию
библиотеках нет.

Добавь `-latomic` — и программа соберётся.

Для `atomic<Pair>` она напечатает `false`: тип не свободен от блокировок,
и библиотека прячет внутри мьютекс. (Детали зависят от компилятора
и платформы, но суть одна: проверка `is_lock_free()` во время выполнения может
потребовать отдельную библиотеку.)
...
`std::atomic_flag` — это булев флаг с двумя состояниями: «установлен» (`true`)
и «сброшен» (`false`). Он намеренно минимален: стандарт даёт ему всего две
операции, потому что флаг задуман как простейший строительный блок.

Создать флаг можно только особым образом — через `ATOMIC_FLAG_INIT`, и в этот
момент он всегда сброшен:

```cpp
std::atomic_flag f = ATOMIC_FLAG_INIT;   // всегда начинается со «сброшен»
```

После создания с флагом можно сделать ровно две вещи:

- **`test_and_set()`** — «установи и верни прежнее». Атомарно переводит флаг
  в `true` и возвращает то значение, которое было до этого. Это операция
  **чтения-изменения-записи** (read-modify-write, RMW): одним неделимым
  действием она и читает старое значение, и записывает новое. Именно поэтому
  результат нельзя получить двумя отдельными шагами «прочитал, потом записал» —
  между ними вклинился бы другой поток.
- **`clear()`** — сбрасывает флаг в `false`.

И всё. Больше с флагом ничего сделать нельзя: его нельзя скопировать или
присвоить, у него нет операции «просто прочитать значение». Например, написать
`bool b = f;` не получится — операции загрузки, которая возвращала бы текущее
состояние, у `atomic_flag` нет.

Оба метода принимают необязательный параметр — **метку упорядочения памяти**
(`std::memory_order`). Она задаёт, как операции одного потока становятся видны
другим. Что означают эти метки и как их выбирать, мы подробно разберём в разделе
«Три модели упорядочения доступа к памяти» ниже — пока просто применяй их «как
есть», не вникая. Если метку не указывать, работает вариант по умолчанию —
самый строгий.

#### Почему атомарные типы не копируются

Копирование атомарного объекта — это на самом деле **две** операции: прочитать
значение из одного объекта и записать его в другой. Каждая по отдельности
атомарна, но вместе они уже нет: между чтением и записью другой поток может
вклиниться и изменить первый объект. Тогда в копии окажется значение, которое
не соответствовало состоянию источника в момент присваивания. Атомарность одной
операции не спасает, когда их две. Поэтому стандарт просто запрещает
копирование и присваивание у атомарных типов — чтобы программист не получил
незаметно некорректную «атомарную» копию.

#### Спин-блокировка на std::atomic_flag

Из-за ограниченности `std::atomic_flag` из него удобно делать **спин-блокировку**
(spinlock) — простой мьютекс на активном ожидании. В её коде встретятся две
метки упорядочения, поэтому забежим вперёд и коротко поясним их (полный разбор —
в разделе «Три модели упорядочения доступа к памяти» ниже).

**`memory_order_release`** («освобождение») — метка для **записи** в атомарную
переменную. Она ставит **односторонний барьер**: все операции с памятью, которые
поток выполнил раньше этой записи, уже не могут «уехать» после неё. Если другой
поток затем прочитает именно это значение, он увидит и все те более ранние
изменения.

**`memory_order_acquire`** («захват») — метка для **чтения** той же переменной.
Барьер здесь зеркальный: операции, которые поток выполнит после этой загрузки,
не могут «уехать» раньше неё. Прочитав значение, записанное с `release`, поток
получает вместе с ним и все изменения, сделанные освободителем до его записи.

Метки работают не по отдельности, а **в паре над одной и той же переменной**:
синхронизация возникает только тогда, когда загрузка с `acquire` прочитала
именно то значение, которое записало сохранение с `release`. Увидел читатель
другое значение (например, старое) — никаких гарантий нет.

Передачу данных удобно представлять как посылку, но важно не понимать метафору
буквально: **в самом флаге ничего не лежит** — он лишь число `0`/`1`. «Внутри
посылки» — это гарантия видимости, а не контейнер. Покажем на примере:

```cpp
int data[100];                  // обычная, неатомарная память
std::atomic<bool> ready(false);

void writer() {
    for (int i = 0; i < 100; ++i) data[i] = i;    // (1) «содержимое посылки»
    ready.store(true, std::memory_order_release); // (2) «запечатали»
}

void reader() {
    while (!ready.load(std::memory_order_acquire)) {} // (3) «распечатали»
    int x = data[42];                                  // (4) гарантированно 42
}
```

«Изменения», о которых идёт речь, — это записи `data[i] = i`, то есть обычные
записи в память, выполненные потоком **до** `store(release)`. Метка `release`
не даёт им переместиться вслед за (2), поэтому (2) становится последней по
времени из этих записей — «крышкой посылки». А `acquire` гарантирует, что
прочитавший это значение поток в (4) увидит всё, что было записано до (2):
между (1) и (4) устанавливается принудительный порядок. Строгие определения
отношений «синхронизируется с» и «происходит до» — в разделе ниже.

Без меток (`relaxed`) читатель мог бы увидеть флаг уже выставленным, а данные —
ещё не готовыми или вперемешку: ни компилятор, ни процессор не обязаны сохранять
порядок записей, к которым обращается только один поток. Именно `release` и
`acquire` делают этот порядок обязательным.

Именно поэтому в спин-блокировке `unlock()` вызывается как `clear(release)`,
а `lock()` — как `test_and_set(acquire)`: разблокировка «запечатывает» всё, что
поток изменил под защитой, а следующая блокировка эти изменения «распечатывает».

Вот сама спин-блокировка:
...
1. Сначала флаг сброшен (`false`) — блокировка свободна.
2. Поток вызывает `lock()` и входит в цикл. `test_and_set()` проверяет флаг
   и тут же ставит его в `true`, возвращая прежнее значение.
   - Вернулось `false` — флаг был свободен, и мы только что заняли его.
     Блокировка наша, выходим из цикла.
   - Вернулось `true` — флаг уже занят другим потоком. Крутимся в цикле
     и снова вызываем `test_and_set()`, пока владелец не освободит флаг.
3. Поток вызывает `unlock()`, делает `clear()` — флаг снова `false`,
   блокировка свободна для следующего.

Такого мьютекса достаточно для `std::lock_guard` (глава 3). Но у активного
ожидания есть цена: поток, которому не досталась блокировка, всё это время
крутится и жжёт процессорное время. Если блокировку удерживают долго или
желающих много, выгоднее настоящий `std::mutex`, который усыпляет ожидающий
поток. Взаимное исключение спин-блокировка обеспечивает; как именно это следует
из порядка операций — подробнее в разделе про упорядочение.
...
Обрати внимание на особенность: оператор присваивания атомарного типа
возвращает **значение** (обычного, неатомарного типа), а не ссылку на объект.
Причина в том, что обычный оператор присваивания возвращает **ссылку** на левый
операнд, то есть на сам объект. Если бы `std::atomic` вёл себя так же,
выражение `b = false` давало бы ссылку на `b`, а не само значение. Чтобы узнать,
что же записалось, пришлось бы ещё раз прочитать `b` — отдельной операцией.
А между записью и этим чтением другой поток мог бы снова изменить `b`, и ты
получил бы не то значение, которое только что туда сохранил. Поэтому атомарные
присваивания возвращают **копию** записанного значения: она уже не связана
с переменной и не может измениться.
...
Сравнение-обмен, CAS (от англ. **compare-and-swap**) — краеугольный камень
атомарного программирования.
...
```cpp
bool expected = false;
// Сравнивает текущее значение атомика с expected.
// Если равны → записывает desired, возвращает true.
// Если не равны → записывает текущее значение атомика в expected, возвращает false.
...
while (!b.compare_exchange_weak(expected, true) && !expected) {
    // повторяем попытку только при ложном сбое
}
```

Зачем нужен `!expected`? Дело в том, что неудачный `compare_exchange_weak`
возвращает `false` по двум разным причинам, и их надо различать.

- **Ложный сбой.** Значения на самом деле совпадали (флаг был `false`), просто
  процессору не удалось выполнить операцию неделимо. Переменная не менялась,
  и `expected` остался прежним — `false`. Тогда `!expected` истинно, и цикл
  повторяет попытку.
- **Настоящий провал.** Оказалось, что `b` уже не равна `expected` — флаг уже
  установлен в `true`. CAS ничего не записывает, но **кладёт фактическое
  значение переменной в `expected`**: теперь `expected == true`. Значит,
  `!expected` ложно, и цикл завершается — нужное состояние (`b == true`) уже
  достигнуто, повторять нечего.

Именно запись фактического значения в `expected` и позволяет условию увидеть,
что флаг уже поднят, и перестать крутиться.
...
У CAS есть особенность: **два параметра упорядочения** — отдельный для случая
успеха и отдельный для случая неудачи. Почему два? Потому что эти случаи ведут
себя по-разному:

- при **успехе** операция и читает, и пишет — применимы все виды упорядочения,
  включая `release`;
- при **неудаче** она только читает: записи `desired` не происходит, а значит,
  и «освобождать» (`release`) нечего — поэтому для неудачи `release` и `acq_rel`
  запрещены.

Отсюда правила:

- упорядочение для неудачи не может быть **строже**, чем для успеха: нельзя
  сделать успешную операцию «слабой», а неуспешную — «сильной»;
- если для неудачи нужно `acquire` или `seq_cst` — то же самое нужно указать
  и для успеха;
- если упорядочение для неудачи не указано, оно выводится из порядка успеха,
  но без части `release`: `release` превращается в `relaxed`, а `acq_rel` —
  в `acquire`;
- если не указано ничего — оба параметра по умолчанию `seq_cst`.

`seq_cst` (`std::memory_order_seq_cst`) — **последовательно согласованное
упорядочение** (sequentially consistent). Это порядок по умолчанию для всех
атомарных операций: он гарантирует, что все потоки видят атомарные операции
в одном общем порядке, как будто они выполнялись по очереди в одном потоке.
Это самая простая для понимания модель, но и самая «дорогая». Подробно —
ниже, в разделе «Три модели упорядочения доступа к памяти».

Благодаря правилу «неудача не строже успеха» следующие два вызова эквивалентны:
...
Во втором случае порядок для неудачи не указан, поэтому он выводится из порядка
успеха: `acq_rel` → `acquire`.

В отличие от `atomic_flag`, `atomic<bool>` может не быть свободен от блокировок
(`is_lock_free()` — см. выше).
...
#### Обобщённый шаблон std::atomic<UDT> (UDT = user-defined type, пользовательский тип)
...
флага `data_ready`, чтобы не жечь процессор. В реальном коде предпочитают
условные переменные из главы 4.)*
...
Код корректен благодаря модели памяти: атомарные операции упорядочивают доступ
к памяти, а атомарный флаг (по умолчанию — `seq_cst`) навязывает принудительный
порядок — запись в `data` происходит до установки флага, поэтому читатель увидит
данные раньше, чем заметит `data_ready == true`. Достаточно одного атомарного
флага, чтобы передать весь вектор.

Как именно метки упорядочения это обеспечивают (отношения «синхронизируется с»
и «происходит до») — в разделе «Три модели упорядочения доступа к памяти» ниже.
...
`relaxed` годится для **независимых** счётчиков, где важен только сам факт
...
**Аналогия с пакетами обновлений.** Представь «людей в кабинках».
При освобождении (release) сохранение помечается как «последнее в пакете»
от потока. При захвате загрузка спрашивает не только значение, но и то,
«последнее ли оно в пакете», и сообщает известные ей пакеты. Тогда захват,
увидев «последнее значение пакета от потока A», может потребовать у кабинки
другой переменной все значения из этого пакета. Так захват-освобождение
«протаскивает» данные через пару операций.
...
что между ними нет отношения «синхронизируется с». Захват-освобождение не даёт глобального порядка:
...
Здесь `assert` никогда не сработает. Почему? Сохранение `x` идёт в коде до
`y.store(true, release)` — освобождение «публикует» всё, что было записано до
него. Читатель делает `y.load(acquire)`: захват увидел значение, записанное
с `release`, а значит, увидел и всё «опубликованное» раньше — включая `x`.
Поэтому нестрогая загрузка `x` обязана прочитать `true`.
...
`consume` — это ослабленный `acquire`, который даёт синхронизацию только для
данных, зависящих от прочитанного значения (**data dependency**). Всё, что не
зависит от этого значения, не синхронизируется. Идея — дать компилятору больше
свободы для оптимизаций, чем `acquire`, но на практике почти никто им не
пользуется: компиляторы всё равно не умеют корректно его реализовывать.
...
void create_x() {
    X* x = new X;
    x->i = 42;                                  // (1) пишем в *x
    x->s = "hello";                             // (2) пишем в *x
    a.store(99, std::memory_order_relaxed);     // (3) независимая запись
    p.store(x, std::memory_order_release);      // (4) публикуем указатель
}

void use_x() {
    X* x;
    while (!(x = p.load(std::memory_order_consume))) { }  // (5) consume
    assert(x->i == 42);                          // (6) ЗАВИСИТ от x → видно
    assert(x->s == "hello");                     // (7) ЗАВИСИТ от x → видно
    assert(a.load(std::memory_order_relaxed) == 99);  // (8) НЕ зависит от x → может не выполниться
}
...
Ключевое — что означает «зависит от x»:

- `x->i` и `x->s` — обращение через указатель `x`, значение которого получено
  из `p.load()`. Значит, `x->i` и `x->s` **зависят** от прочитанного значения
  `p` — эти данные синхронизируются с `release`-записью в писателе;
- `a` — отдельная переменная, никак не связана с `x`. Её загрузка не зависит
  от значения `p` — синхронизации нет, `a.load(relaxed)` может вернуть `0`.

**Что такое data dependency.** Зависимость по данным — ситуация, когда значение,
которое ты читаешь или вычисляешь, зависит от ранее прочитанного значения.

Примеры зависимостей:

```cpp
X* x = p.load(std::memory_order_consume);
x->i          // зависит от x → синхронизировано
x->s          // зависит от x → синхронизировано
(*x).i        // зависит от x → синхронизировано
```

Примеры отсутствия зависимости:

```cpp
a.load()      // не зависит от x → НЕ синхронизировано
b + c         // не зависит от x → НЕ синхронизировано
```
...
Отношение «синхронизируется с» (synchronizes-with) — это парная связь между:

- release-записью в одном потоке (store, RMW),
- acquire-чтением в другом потоке (load, RMW), которое прочитало значение,
  записанное этой release-записью.

Когда оно установлено, все записи, сделанные до release в первом потоке,
становятся видимы второму потоку после acquire.

Наивно кажется: чтобы синхронизация сработала, acquire-чтение должно прочитать
именно то значение, которое записал release-store. Но это не так — и здесь
в игру вступает release sequence.

**Проблема: цепочка RMW между store и load.** Представь:

```cpp
std::atomic<int> count{0};

// Поток A
count.store(20, std::memory_order_release);   // (1) release

// Поток B
int x = count.fetch_sub(1, std::memory_order_acquire);   // (2) acquire-RMW

// Поток C
int y = count.fetch_sub(1, std::memory_order_acquire);   // (3) acquire-RMW
```

Поток B прочитал 20 — то самое значение, что записал store (1). → Синхронизация (1) ↔ (2) есть.

Поток C прочитал 19 — значение, записанное не store (1), а fetch_sub (2). → Наивно кажется, что синхронизации (1) ↔ (3) нет.

Но если бы её не было, поток C не увидел бы данные, опубликованные потоком A до release. А это нужно — иначе чтение буфера было бы гонкой.

Release sequence решает эту проблему.
...
| [5.3. Атомарный счётчик через CAS](labs/lab-05-03-cas-counter/task.md) | напиши с нуля | реализуй атомарную структуру на `compare_exchange_strong` без `fetch_add` |
| [5.4. Барьеры памяти](labs/lab-05-04-fences/task.md) | допиши TODO | расставь release/acquire барьеры (`atomic_thread_fence`), чтобы упорядочить неатомарные данные |
| [5.5. Публикация через `atomic<T*>`](labs/lab-05-05-publish-pointer/task.md) | напиши с нуля | опубликуй объект через атомарный указатель с парой release/acquire для нескольких читателей |
| [5.6. Партия через release sequence](labs/lab-05-06-release-sequence/task.md) | допиши TODO | раздай задания потребителям через `store(release)` + `fetch_sub(acquire)` |
