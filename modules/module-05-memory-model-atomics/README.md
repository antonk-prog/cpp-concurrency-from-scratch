...
Оба метода принимают метку упорядочения `memory_order`; по умолчанию
используется самый строгий — `memory_order_seq_cst`. Но есть ограничение:
`clear()` — это чистая **запись** (в стандарте — store, «сохранение»), а не
чтение. Поэтому ей нельзя передать `acquire` или `acq_rel`: эти метки описывают,
как поток **читает** чужие изменения, а у простой записи читать нечего.
