...
Что это даёт на практике? Для каждой отдельной переменной все потоки видят
одни и те же записи, выстроенные в эту цепочку. Поэтому если поток однажды
прочитал некоторое значение, то при следующих чтениях той же переменной он
увидит это же значение или более позднее по цепочке, но никогда — более раннее.
Так же и с записью: она встаёт в цепочку после всех изменений, которые поток
уже наблюдал, и «вернуть назад» их для него невозможно. Кратко: значения одной
переменной в одном потоке не откатываются назад — они движутся только вперёд
по общей для всех потоков цепочке.
