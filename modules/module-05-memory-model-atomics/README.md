...
переменную. Она «прикладывает» к этой записи всё, что поток успел сделать
раньше: если другой поток затем прочитает именно это значение, он увидит и все
те более ранние изменения.

**`memory_order_acquire`** («захват») — метка для **чтения** той же переменной.
Она делает обратное: прочитав значение, записанное с `release`, поток получает
вместе с ним все изменения, которые освободитель сделал до своей записи.

Наглядно это удобно представить как передачу посылки. Поток-писатель сначала
готовит данные, а последним шагом выставляет атомарный флаг с `release` —
«запечатывает посылку»: все сделанные до этого изменения оказываются внутри.
Поток-читатель читает флаг с `acquire` — «распечатывает посылку»: увидев нужное
значение, он получает и всё её содержимое. Без этих меток читатель мог бы
заметить флаг уже выставленным, но сами данные — ещё не готовыми (или увидеть
их вперемешку).
