...
То же и с загрузкой: если сама загрузка объекта атомарна и все записи в него
тоже атомарны, чтение вернёт целиком либо исходное значение, либо значение одной
из записей — но не «полузаписанную смесь». Сравни: обычное присваивание структуре
из нескольких полей — составная операция, и другой поток может увидеть часть
полей уже обновлёнными, а часть — ещё старыми. Именно так возникает «смесь»,
а несинхронизированный доступ к неатомарной переменной из нескольких потоков —
это гонка за данными.
