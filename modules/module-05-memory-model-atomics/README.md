...
важен перфоманс и низкоуровневый контроль. Мьютекс — «дорогой» примитив: он
...
- **мьютекс** (глава 3): если один и тот же мьютекс блокируется перед обоими
  обращениями, в конкретный момент времени к области обращается один поток,
  значит, одно обращение строго раньше другого;
- **атомарные операции** (эта глава): операции над атомарными переменными
  могут навязать порядок и для других (даже неатомарных) областей памяти.
...
Что это даёт на практике? Если поток увидел конкретное значение объекта,
последующие чтения в том же потоке не могут вернуть «более старое» значение,
а последующие записи обязаны встать позже в модификационном порядке. Проще
говоря: значения одной переменной «не откатываются назад» в одном потоке.
...
**Атомарная операция** — неделимая. Ни один поток не может наблюдать её
«на полпути»: она либо выполнена целиком, либо ещё не началась. Если операция
загрузки, читающая объект, атомарна, и все изменения этого объекта тоже
атомарны, то загрузка вернёт либо исходное значение, либо значение, сохранённое
одной из записей, — но никогда «полузаписанную смесь».

Сравни с неатомарной операцией. Например, присваивание структуре из нескольких
полей — составная операция. Другой поток может увидеть часть полей обновлённой,
а часть — ещё нет. Так и возникает «смесь» значений, и несинхронизированный
доступ к неатомарной переменной из нескольких потоков — гонка за данными.
...
В Concurrency TS есть и тип `std::experimental::atomic_shared_ptr<T>` — атомарная
обёртка над `shared_ptr`, дающая те же операции (`load`, `store`, `exchange`,
CAS). Её предпочитают свободным функциям, потому что код становится понятнее
и все обращения гарантированно атомарны. Но, как и у других атомарных типов,
стоит проверять `is_lock_free()` и профилировать.
...
|------|-----|-----------|--------------------|
| [5.1. Спин-блокировка на atomic_flag](labs/lab-05-01-spinlock/task.md) | допиши TODO | базовая | реализуй `lock`/`unlock` через `test_and_set`/`clear` и защити общий счётчик |
| [5.2. Почини relaxed-гонку](labs/lab-05-02-relaxed-ordering/task.md) | найди и почини | средняя | исправь нестрогое упорядочение, чтобы данные передавались корректно |
| [5.3. Атомарный счётчик через CAS](labs/lab-05-03-cas-counter/task.md) | напиши с нуля | сложная | реализуй атомарную структуру на `compare_exchange_weak` без `fetch_add` |
