...
// Тест «один push + один pop на пустой очереди» через go-барьер.
bool test_concurrent_push_and_pop_on_empty_queue() {
    ...
    try {
        push_done = std::async(std::launch::async, [&q, ready, &push_ready] {
            ...
            ready.wait();
            // TODO: положить значение 42 в очередь: q.push(42);
        });
        pop_done = std::async(std::launch::async, [&q, ready, &pop_ready] {
            pop_ready.set_value();
            ready.wait();
            // TODO: извлечь значение из очереди и вернуть его: return q.pop();
            return 0;
        });
        ...
        push_done.get();
        // TODO: проверки: извлечённое значение == 42; очередь пуста.
