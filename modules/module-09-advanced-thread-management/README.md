...
|------|-----|-----------|--------------------|
| [9.1. Простой пул потоков](labs/lab-09-01-simple-thread-pool/task.md) | напиши с нуля | средняя | реализуй пул потоков: очередь задач, `submit`, флаг `done`, join всех |
| [9.2. Задача ждёт задачу](labs/lab-09-02-task-waits-task/task.md) | предскажи вывод | сложная | объясни, почему зависает пул, когда задача ждёт результата другой задачи |
| [9.3. Гонка за флаг прерывания](labs/lab-09-03-interrupt-flag-race/task.md) | найди и почини | средняя | почини неатомарный флаг в `interruptible_thread` |