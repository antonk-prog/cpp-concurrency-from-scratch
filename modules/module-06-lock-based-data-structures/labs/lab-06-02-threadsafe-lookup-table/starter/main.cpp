...
// Потокобезопасная поисковая таблица: хеш-таблица с сегментами,
// каждый защищён std::shared_mutex.
// TODO: допиши три метода bucket_type:
//   - value_for                (под shared_lock)
//   - add_or_update_mapping    (под unique_lock)
//   - remove_mapping           (под unique_lock)
...
class threadsafe_lookup_table {
    ...
    public:
        ...
        Value value_for(Key const& key, Value const& default_value) const {
            // TODO: shared_lock, найти запись, вернуть значение или default_value
            (void)key;
            return default_value;
        }

        void add_or_update_mapping(Key const& key, Value const& value) {
            // TODO: unique_lock, найти запись:
            //   нет ключа -> push_back(pair(key,value))
            //   есть ключ  -> обновить значение
            (void)key;
            (void)value;
        }

        void remove_mapping(Key const& key) {
            // TODO: unique_lock, найти запись, при наличии erase
            (void)key;
        }
    };
